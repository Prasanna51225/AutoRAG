...
class EmbeddingCache:
    ...
    def _key(self, text: str) -> str:
        # Use MD5 to keep key length reasonable
...
class DenseEmbedder:
    """Generate dense embeddings using sentence-transformers with Redis cache."""
    
    def __init__(self, model_name: str = "sentence-transformers/all-MiniLM-L6-v2"):
        logger.info(f"Loading dense embedding model: {model_name}")
        self.model = SentenceTransformer(model_name)
        self.dimension = self.model.get_sentence_embedding_dimension()
        # Redis cache (initialised later via set_redis)
    ...
    async def embed_batch(self, texts: List[str]) -> List[List[float]]:
        """Embed multiple texts efficiently (no per-item threading overhead)."""
